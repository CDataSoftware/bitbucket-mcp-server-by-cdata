...
CData's Model Context Protocol (MCP) Server for Bitbucket

:heavy_exclamation_mark: This project builds a read-only MCP server. For full read, write, update, delete, and action capabilities and a simplified setup, check out our free [CData MCP Server for Bitbucket (beta)](https://www.cdata.com/download/download.aspx?sku=KJZM-V&type=beta). 
## Purpose
We created this read-only MCP Server to allow LLMs (like Claude Desktop) to query live data Bitbucket supported by the [CData JDBC Driver for Bitbucket](https://www.cdata.com/drivers/bitbucket/jdbc).

CData JDBC Driver connects to Bitbucket by exposing them as relational SQL models.

This server wraps that driver and makes Bitbucket data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.

## Setup Guide
1. Clone the repository:
      ```bash
      git clone https://github.com/cdatasoftware/bitbucket-mcp-server-by-cdata.git
      cd bitbucket-mcp-server-by-cdata
      ```
2. Build the server:
      ```bash
      mvn clean install
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/bitbucket/download/jdbc](https://www.cdata.com/drivers/bitbucket/download/jdbc)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Bitbucket\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Bitbucket/`
    * Run the command `java -jar cdata.jdbc.bitbucket.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.bitbucket.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `bitbucket.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.bitbucket.BitbucketDriver)
    * **JdbcUrl** - the JDBC connection string to use with the CData JDBC Driver to connect to your data (copied from above)
    * **Tables** - leave blank to access all data, otherwise you can explicitly declare the tables you wish to create access for
      ```env
      Prefix=bitbucket
      ServerName=CDataBitbucket
      ServerVersion=1.0
      DriverPath=PATH\TO\cdata.jdbc.bitbucket.jar
      DriverClass=cdata.jdbc.bitbucket.BitbucketDriver
      JdbcUrl=jdbc:bitbucket:InitiateOAuth=GETANDREFRESH;
      Tables=
      ```
